...
class CloudflareScraper(Session):
    def __init__(self, episodeSearching=False, *args, **kwargs):
        super(CloudflareScraper, self).__init__(*args, **kwargs)

        self.episodeSearching = episodeSearching

        if "requests" in self.headers["User-Agent"]:
            # Spoof Firefox on Linux if no custom User-Agent has been set
            self.headers["User-Agent"] = DEFAULT_USER_AGENT

    def request(self, method, url, cookie=None, *args, **kwargs):
        resp = super(CloudflareScraper, self).request(method, url, *args, **kwargs)

        # Check if Cloudflare anti-bot is on
        if resp.status_code == 503 and resp.headers.get("Server") == "cloudflare-nginx":
            if self.episodeSearching:
                # Return response from "wait for 5 seconds" page in order to read certain headers
                # as well as cf bypassed response.
                return (resp, self.solve_cf_challenge(resp, **kwargs))
            return self.solve_cf_challenge(resp, cookie=cookie, **kwargs)

        # Otherwise, no Cloudflare anti-bot detected
        return resp

    def solve_cf_challenge(self, resp, cookie=None, **kwargs):
    ...
    @classmethod
    def create_scraper(cls, sess=None, episodeSearching=False, **kwargs):
        """
        Convenience function for creating a ready-to-go requests.Session (subclass) object.
        """

        scraper = cls(episodeSearching=episodeSearching)
